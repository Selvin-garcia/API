const { sequelize, Rol } = require("../db.js");
 ...
 const DatosJson = JSON.stringify(req.body);
  try {
      ...
      const resultado = await sequelize.query(
          'EXEC CrearRoles @DatosJson = :DatosJson',
          {
...
exports.obtenerRoles = async (req, res, next) => {
 ...
 const DatosJson = JSON.stringify(req.body);
  try {
      ...
      // Llamar al procedimiento almacenado para crear el rol y devolver el resultado
      const resultado = await sequelize.query(
          'EXEC ObtenerRoles @DatosJson = :DatosJson',
          {
              replacements: { DatosJson },
...
exports.actualizarRoles = async (req, res, next) => {
  // Obtener los datos JSON del cuerpo de la solicitud POST 
  const DatosJson = JSON.stringify(req.body);
   try {
       ...
       // Llamar al procedimiento almacenado para crear el rol y devolver el resultado
       const resultado = await sequelize.query(
           'EXEC ActualizarRoles @DatosJson = :DatosJson',
           {
               replacements: { DatosJson },
               type: sequelize.QueryTypes.RAW
           }
