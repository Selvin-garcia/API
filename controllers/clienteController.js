const { sequelize, Cliente } = require("../db.js");

exports.crearClientes = async (req, res, next) => {
 ...
 const DatosJson = JSON.stringify(req.body);
  try {
      ...
      const resultado = await sequelize.query(
          'EXEC CrearClientes @DatosJson = :DatosJson',
          {
...
exports.obtenerClientes = async (req, res, next) => {
 ...
 const DatosJson = JSON.stringify(req.body);
  try {
      ...
      // Llamar al procedimiento almacenado para crear el rol y devolver el resultado
      const resultado = await sequelize.query(
          'EXEC ObtenerClientes @DatosJson = :DatosJson',
          {
              replacements: { DatosJson },
...
exports.actualizarClientes = async (req, res, next) => {
  // Obtener los datos JSON del cuerpo de la solicitud POST 
  const DatosJson = JSON.stringify(req.body);
   try {
       ...
       // Llamar al procedimiento almacenado para crear el rol y devolver el resultado
       const resultado = await sequelize.query(
           'EXEC ActualizarClientes @DatosJson = :DatosJson',
           {
               replacements: { DatosJson },
               type: sequelize.QueryTypes.RAW
           }
