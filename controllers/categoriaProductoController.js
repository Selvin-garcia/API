const { sequelize, CategoriaProducto } = require("../db.js");

exports.crearCategoriaProductos = async (req, res, next) => {
 ...
 const DatosJson = JSON.stringify(req.body);
  try {
      ...
      const resultado = await sequelize.query(
          'EXEC CrearCategoriaProductos @DatosJson = :DatosJson',
          {
...
exports.obtenerCategoriaProductos = async (req, res, next) => {
 // Obtener los datos JSON del cuerpo de la solicitud POST 
 const DatosJson = JSON.stringify(req.body);
  try {
      ...
      // Llamar al procedimiento almacenado para crear el rol y devolver el resultado
      const resultado = await sequelize.query(
          'EXEC ObtenerCategoriaProductos @DatosJson = :DatosJson',
          {
              replacements: { DatosJson },
...
exports.actualizarCategoriaProductos = async (req, res, next) => {
  // Obtener los datos JSON del cuerpo de la solicitud POST 
  const DatosJson = JSON.stringify(req.body);
   try {
       ...
       // Llamar al procedimiento almacenado para crear el rol y devolver el resultado
       const resultado = await sequelize.query(
           'EXEC ActualizarCategoriaProductos @DatosJson = :DatosJson',
           {
               replacements: { DatosJson },
               type: sequelize.QueryTypes.RAW
           }
